...
const ProductsProvider = ({children}) => {
  const [products, setProducts] = useState<Realm.Results<Products> | []>([]);

  const realmRef = useRef<Realm | null>(null);

  const subscriptionRef = useRef<Realm.Results<Products> | null>(null);
  ...
  const openRealm = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      //   schema: [Products.schema, Products.ProductCategorySchema],
      //   schemaVersion: 1,
      ...
      const productsResults: Realm.Results<Products> =
        realm.objects('Products');
      ...
      subscriptionRef.current = productsResults;
      productsResults.addListener((/*collection, changes*/) => {
        ...
        setProducts(realm.objects('Products'));
      });
    } catch (err: any) {
      console.error('Error opening realm: ', err.message);
    }
  }, [realmRef, setProducts, user]);

  const closeProductRealm = useCallback((): void => {
  ...
  useEffect(() => {
    ...
    return closeProductRealm;
  }, [openRealm, closeProductRealm, user]);

  const handleAddProduct = useCallback(
    async (data: any, shopId: any, agencyId?: any): any => {
      if (!data) {
        return;
      }
      ...
      const shop = realm?.objectForPrimaryKey(
        'Shop',
        new Realm.BSON.ObjectId(shopId),
      );

      const agency = realm?.objectForPrimaryKey(
        'Agency',
        new Realm.BSON.ObjectId(agencyId),
      );

      const product = await realm?.write(() => {
        return realm?.create('Products', Products.generate(data));
      });
      ...
      await realm?.write(() => {
        realm?.create(
          'Shop',
          {
            _id: new Realm.BSON.ObjectId(shopId),
      ...
      await realm?.write(() => {
        realm?.create(
          'Agency',
          {
            _id: new Realm.BSON.ObjectId(agencyId),
            products: [...agency.products, product],
            updated_at: new Date(),
          },
          Realm.UpdateMode.Modified,
        );
...
const useProducts = () => {
  ...
  if (product == null) {
    throw new Error('useProducts() called outside of a UsersProvider?'); // an alert is not placed because this is an error for the developer not the user
  }
  return product;
};
