...
const OrderContext = React.createContext<any>(null);

// const appRealm = getRealm();

const OrderProvider = ({children}) => {
  const [orders, setOrders] = useState<Realm.Results<Order> | []>([]);
  const [selectedOrder, setSelectedOrder] = useState<
    Realm.Results<Order> | undefined | null
  >(null);

  const realmRef = useRef<Realm | null>(null);

  const subscriptionRef = useRef<Realm.Results<Order> | null>(null);
  ...
  const openRealm = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const OrderResults: Realm.Results<Order> = realm.objects('Order');
      if (OrderResults?.length) {
        setOrders(OrderResults);
      }

      // Live queries and objects emit notifications when something has changed that we can listen for.
      subscriptionRef.current = OrderResults;
      OrderResults.addListener((/*collection, changes*/) => {
        ...
        setOrders(realm.objects('Order'));
      });
    } catch (err: any) {
      console.error('Error opening realm: ', err.message);
    }
  }, [realmRef, setOrders, user]);

  const closeOrderRealm = useCallback((): void => {
  ...
  useEffect(() => {
    ...
    return closeOrderRealm;
  }, [openRealm, closeOrderRealm, user]);
  ...
  const handleAddOrder = useCallback(
    async (data: any): Promise<any> => {
      ...
      const realm = realmRef.current;
      return realm?.write(() => {
        return realm?.create('Order', Order.generate(data));
      });
    },
    [realmRef],
  );
  ...
  const handleOrderEdit = useCallback(
    (id: string, data: any): void => {
      ...
      realm?.write(() => {
        ...
        let oldShop = realm?.objectForPrimaryKey(
          'Order',
  ...
  const handleGetOneOrder = useCallback(
    (id: string): any => {
      const realm = realmRef.current;
      return realm?.objectForPrimaryKey('Order', new Realm.BSON.ObjectId(id));
