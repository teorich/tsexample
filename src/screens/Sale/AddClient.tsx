...
const productReducer = (prevState, action) => {
  ...
  switch (action.type) {
    case 'INIT':
      array = action.payload.map(item => ({product: item, quantity: 1}));
      return array;
    case 'ADD':
      array = [...prevState];
  ...
  return (
    <SafeAreaView style={tw.style('flex-1 bg-[#E5E9F2]')}>
      <CustomHeader
        ...
        onSelect={() => {}}
      />
      <View style={tw.style('flex bg-white items-center')}>
        <Text
          style={tw.style(
            ' m-auto text-xl tracking-wide font-medium leading-5 m-5',
          )}>
          Ajouter un Client{' '}
        </Text>
      </View>
